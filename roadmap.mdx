...
<CardGroup cols={2}>
  <Card title="Player Rappelling" icon="person-falling">
    Crew members aboard the UH-1H and Blackhawk will be able to rappel from the aircraft while it holds a hover. Designed to work alongside Auto Hover — pilot locks position, crew deploy to the ground on ropes.
  </Card>
  ...
  <Card title="Winch System" icon="arrow-up-from-bracket">
    A deployable winch for the UH-1H and Blackhawk that allows extraction of players or cargo from the ground while in a stable hover. Planned to integrate with the existing Cabin Mode crew system.
  </Card>
...
<CardGroup cols={2}>
  ...
  <Card title="Damage Visual States" icon="fire">
    Visible smoke trails and fire effects tied to helicopter damage zone health — a heavily damaged engine zone produces visible smoke, escalating to fire before destruction. Component deformation is also under consideration.
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  <Card title="Loot Table Integration" icon="box-open">
    A structured spawn table for `HelicopterBattery` and `HelicopterRepairKit` with configurable rarity tiers per map zone — removing the need for manual types.xml tuning.
  </Card>
  <Card title="Admin Spawn Menu Support" icon="terminal">
    Native compatibility entries for popular admin tools, allowing all helicopter variants to appear correctly in third-party admin spawn menus without manual classname lookup.
  </Card>
</CardGroup>
