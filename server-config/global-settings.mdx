...
| `PILOT_ENTER` | Player takes the pilot seat |
| `PILOT_EXIT` | Player leaves the pilot seat |
| `PILOT_KILLED` | Pilot is killed while seated |
| `COPILOT_ENTER` | Player takes the co-pilot seat |
| `COPILOT_EXIT` | Player leaves the co-pilot seat |
| `CABIN_ENTER` | Pilot or co-pilot enters Interior Walk mode (UH-1H / Blackhawk) |
| `CABIN_EXIT` | Player returns to cockpit from Interior Walk mode |
...
| `ITEM_ATTACHED` | Item placed into a helicopter inventory slot |
| `ITEM_DETACHED` | Item removed from a helicopter inventory slot |
| `CRASH` | Hard impact — helicopter sustains crash damage |
| `WATER_SLAM` | High-speed water contact |
| `ROTOR_SHEAR` | Main or tail rotor blade sheared off on geometry |
| `ROTOR_PLAYER_KILL` | A player is killed by contact with a spinning rotor disc |
| `FLIP_RECOVERY` | Ground flip recovery triggered (requires `CZH_groundFlipRecovery = 1`) |
| `DESTROYED` | Helicopter is destroyed |
...
<Note>
  Autorotation entry and exit events (`AUTOROT_ENTRY` / `AUTOROT_EXIT`) are written to the **debug log** (`CZAutoz_Heli_debug.log`) rather than the activity log, and only when `CZH_debugLogging = 1`.
</Note>
