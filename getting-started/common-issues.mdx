...
<AccordionGroup>
  ...
  <Accordion title="Autopilot X key does nothing">
...
<AccordionGroup>
  <Accordion title="Cannot repair the helicopter">
    All repairs require:
...
<AccordionGroup>
  ...
  <Accordion title="Interior Walk is disabled server-wide">
...
For diagnosing in-mod issues (repair actions not appearing, config not loading, physics anomalies). This log is **only written when debug logging is enabled** — it is off by default.
...
5. Open `$profile:\CZAutoz_Heli_debug.log`

<Warning>
  Turn `CZH_debugLogging` back to `0` once you are done. Leaving it on permanently generates large log files and can impact server performance.
</Warning>
...
A daily timestamped record of all player helicopter activity — engine starts/stops, refuels, repairs, ramp and window operations. Each entry includes the player's display name, Steam GUID, and helicopter classname. Enabled by setting `CZH_activityLogging = 1` in `mod_config.json`.

See [Global Settings](/server-config/global-settings#logging) for the full list of recorded events.
...
<Card title="CZ Autoz Discord" icon="discord" href="https://discord.gg/czautoz">
