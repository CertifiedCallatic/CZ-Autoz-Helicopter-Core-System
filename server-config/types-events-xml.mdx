...
<Tip>
  Adjust the `nominal`, `min`, and `restock` values for `CZ_HelicopterBattery` and `HelicopterRepairKit` to match how commonly you want them available on your server. The values above are a reasonable starting point for a mid-rarity loot economy.
</Tip>
